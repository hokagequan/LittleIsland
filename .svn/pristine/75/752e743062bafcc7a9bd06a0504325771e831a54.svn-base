...
- (id)initWithString:(NSString *)string position:(CGPoint)position isAnswer:(BOOL)isAnswer {
    SKTextureAtlas *atlas = [SKTextureAtlas atlasNamed:[UniversalUtil universalAtlasName:@"CGBallonRed_Death"]];
    ...
    if (self = [super initWithTexture:theTexture string:string position:position isAnswer:isAnswer]) {
        ...
        self.label.position = CGPointMake(self.label.position.x, self.label.position.y + [UniversalUtil universalDelta:30]);
    }
...
+ (void)loadAssets {
    deathAnimationFrames = [GlobalUtil loadFramesFromAtlas:[UniversalUtil universalAtlasName:@"CGBallonRed_Death"]
                                              baseFileName:@"cgballon_red_death_"
                                                  frameNum:10];
}
